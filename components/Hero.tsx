  ...
  ({ level, size }: { level: number; size: number }) => {
    ...
    const Stage1 = (
      <svg {...svgProps}>
        ...
        <g transform="translate(12 11)">
          <ellipse cx="-2" cy="-2" rx="2.2" ry="1.2" fill="#66BB6A" transform="rotate(-35)" />
          <ellipse cx="2" cy="-2" rx="2.2" ry="1.2" fill="#66BB6A" transform="rotate(35)" />
        </g>
      </svg>
    );
    ...
    const Stage2 = (
      <svg {...svgProps}>
        ...
        <g transform="translate(12 10)">
          <ellipse cx="-3" cy="0" rx="3" ry="1.6" fill="#81C784" transform="rotate(-40)" />
          <ellipse cx="3" cy="0" rx="3" ry="1.6" fill="#81C784" transform="rotate(40)" />
    ...
    const Stage3 = (
      <svg {...svgProps}>
        ...
        {/* Leaves */}
        <g transform="translate(12 10)">
          <ellipse cx="-3.5" cy="0.5" rx="3" ry="1.6" fill="#66BB6A" transform="rotate(-45)" />
          <ellipse cx="3.5" cy="0.5" rx="3" ry="1.6" fill="#66BB6A" transform="rotate(45)" />
    ...
    const Stage4 = (
      <svg {...svgProps}>
        ...
        <ellipse cx="17" cy="8" rx="3" ry="1.6" fill="#F06292" transform="rotate(72 17 8)" />
        <ellipse cx="15" cy="15" rx="3" ry="1.6" fill="#F06292" transform="rotate(144 15 15)" />
        <ellipse cx="9" cy="15" rx="3" ry="1.6" fill="#F06292" transform="rotate(216 9 15)" />
        <ellipse cx="7" cy="8" rx="3" ry="1.6" fill="#F06292" transform="rotate(288 7 8)" />
        {/* Center */}
        <circle cx="12" cy="10" r="2.2" fill="#FFC1E3" />
        {/* Sparkle */}
        <g transform="translate(18 6) rotate(15)">
          <line x1="-1.2" y1="0" x2="1.2" y2="0" stroke="#FFD700" strokeWidth="0.6" />
          <line x1="0" y1="-1.2" x2="0" y2="1.2" stroke="#FFD700" strokeWidth="0.6" />
        </g>
      </svg>
    );
...
const CalendarDay = memo(({ 
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: day.isCurrentYear ? (dimmed ? 0.2 : 1) : 0.4, scale: 1 }}
      transition={{
        duration: 0.2,
      ...
      style={{
        width: blockSize + 'px',
        height: blockSize + 'px',
        backgroundColor: day.isCurrentYear ? stage.backgroundColor : 'rgba(141, 110, 99, 0.3)',
        border: day.isCurrentYear ? stage.border : '2px solid rgba(109, 76, 65, 0.3)',
        boxShadow: day.isCurrentYear 
          ? `0 2px 6px ${getGardenShadow(day.level)}, inset 0 1px 2px rgba(255, 255, 255, 0.3)`
          : '0 1px 3px rgba(109, 76, 65, 0.2)',
        display: 'flex',
        alignItems: 'center',
        justifyContent: 'center',
      }}
      title={`${day.date}: ${day.count} contributions - ${day.isCurrentYear ? stage.description : 'Dormant season'}`}
      whileHover={{ 
        scale: 1.1,
        y: -1,
        transition: { 
...
const AnimatedGitHubCalendar = memo(({ contributions }: { contributions: any[] }) => {
      ...
      <div className="relative flex flex-col items-center gap-4 p-8">
        ...
        <div className="flex gap-2">
              ...
              return (
                <motion.button
                  ...
                  style={{
                    ...
                    color: active ? 'rgba(31, 36, 171, 0.9)' : 'rgba(31, 36, 171, 0.5)',
          ...
          <div className="p-4 rounded-2xl" style={{
            background: 'linear-gradient(135deg, rgba(255, 248, 225, 0.85), rgba(245, 245, 220, 0.7))',
            boxShadow: 'inset 0 2px 4px rgba(139, 69, 19, 0.1), 0 4px 12px rgba(139, 69, 19, 0.08)',
        ...
        >
          <span className="text-xs font-medium" style={{ color: '#5D4037' }}>Garden Growth:</span>
          <div className="flex gap-2">
            {[0, 1, 2, 3, 4].map((level) => {
                ...
                >
                  <div
                    ...
                    style={{
                      ...
                      borderColor: stage.border.match(/#[0-9A-F]{6}/i)?.[0] || '#6D4C41',
                  ...
                  <span className="text-xs text-center" style={{ color: '#5D4037', fontSize: '10px' }}>
                    {level === 0 ? 'Empty' : 
                     level === 1 ? 'Sprout' :
                     level === 2 ? 'Growing' :
                     level === 3 ? 'Blooming' : 'Full Bloom'}
                  </span>
                </div>
              );
            })}
          ...
          >
            ...
            >
              {contributions?.contributions && (
                <AnimatedGitHubCalendar contributions={contributions.contributions} />
              )}
            </motion.div>
